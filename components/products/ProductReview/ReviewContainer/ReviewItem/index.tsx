...
import IcoThumbDown from '@/components/icons/IcoThumbDown'
...
const ReviewItem = ({ reviews }: ReviewContainerProps) => {
  if (!reviews || reviews.length === 0) return null
  return (
    <div className="mt-8 md:mt-12 pt-8 md:pt-12 border-t-2">
      {reviews.map((r) => (
        <div key={r.id} className="mb-6 border-b pb-6">
          <div className="grid grid-cols-1 md:grid-cols-5 md:gap-16">
            <div className="md:col-span-3">
              ...
              <div>
                <p className="font-medium text-xl mt-2">{r.headline}</p>
                <ExpandableText
                  text={r.comment}
                  lineClamp={6}
                  className="mt-4"
              ...
              {r.quality && r.value && (
                <div className="space-y-5 mt-5 md:mt-0">
                  ...
                  <div className='md:mt-8'>
                    <RatingBar label="Value of this product" value={r.value} />
                  </div>
                </div>
              )}
              {r.recommend && (
